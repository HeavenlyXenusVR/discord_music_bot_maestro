    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                try: await cur.execute("ALTER TABLE maestro_playback_state ADD COLUMN title TEXT")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_playback_state ADD COLUMN is_paused BOOLEAN DEFAULT FALSE")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE maestro_guild_settings MODIFY loop_mode VARCHAR(10) DEFAULT 'queue'")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_guild_settings ADD COLUMN fade_seconds FLOAT DEFAULT 5.0")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_guild_settings ADD COLUMN fade_curve VARCHAR(20) DEFAULT 'linear'")
                except: pass
                try: await cur.execute("UPDATE maestro_guild_settings SET loop_mode = 'queue' WHERE loop_mode IS NULL OR loop_mode NOT IN ('off', 'song', 'queue')")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE maestro_queue ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_queue_backup ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_queue ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_queue_backup ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_history ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX maestro_track_intelligence_recent_idx ON maestro_track_intelligence (guild_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_track_intelligence_requester_idx ON maestro_track_intelligence (guild_id, last_requester_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_user_affinity_recent_idx ON maestro_user_track_affinity (guild_id, user_id, last_requested)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_smart_recommendations_recent_idx ON maestro_smart_recommendations (guild_id, created_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE maestro_voice_state ADD COLUMN reconnect_attempts INT NOT NULL DEFAULT 0")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_voice_state ADD COLUMN last_error TEXT NULL")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_voice_state_rejoin_idx ON maestro_voice_state (bot_name, desired_connected, last_seen_at)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_metrics_status_idx ON maestro_metrics (bot_name, updated_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE maestro_active_playlists ADD COLUMN channel_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX maestro_queue_lookup_idx ON maestro_queue (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_queue_backup_lookup_idx ON maestro_queue_backup (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_history_guild_played_idx ON maestro_history (guild_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_history_requester_idx ON maestro_history (guild_id, requester_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_playback_resume_idx ON maestro_playback_state (bot_name, is_playing, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_playlist_bot_idx ON maestro_active_playlists (bot_name, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX maestro_user_playlist_lookup_idx ON maestro_user_playlists (user_id, playlist_name)")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_playback_state ADD COLUMN bot_name VARCHAR(50) DEFAULT 'maestro'")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_active_playlists ADD COLUMN bot_name VARCHAR(50) DEFAULT 'maestro'")
                except: pass
                try: await cur.execute("ALTER TABLE maestro_bot_home_channels ADD COLUMN bot_name VARCHAR(50) DEFAULT 'maestro'")
                except: pass
...
@bot.tree.command(name="maestro_main_shuffle", description="Randomize the order of the upcoming queue while keeping the currently playing track untouched.")
async def shuffle(interaction: discord.Interaction):
    if not await is_dj(interaction): return
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id FROM maestro_queue WHERE guild_id = %s AND bot_name = 'maestro'", (interaction.guild.id,))
