...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM maestro_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM maestro_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 18", (guild.id,))
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        await cur.execute("SELECT COUNT(*) AS total FROM maestro_queue WHERE guild_id = %s AND bot_name = 'maestro'", (g.id,))
                        queue_row = await cur.fetchone() or {}
                        await cur.execute("SELECT COUNT(*) AS total FROM maestro_queue_backup WHERE guild_id = %s AND bot_name = 'maestro'", (g.id,))
                        backup_row = await cur.fetchone() or {}
                        await cur.execute("SELECT is_playing, is_paused, position_seconds FROM maestro_playback_state WHERE guild_id = %s AND bot_name = 'maestro' LIMIT 1", (g.id,))
