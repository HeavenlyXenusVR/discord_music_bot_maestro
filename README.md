...
![Maestro bot icon](assets/bot-icon.png)

Maestro is one node in the 13-bot Discord music fleet. Runs the show for servers that want a conductor at the helm. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Maestro)**
- Internal node name: `maestro`
- Command namespace: `/maestro_main_*`
...
All commands are namespaced as `/maestro_main_<command>`.
...
- Node: `maestro`
- Database schema: `discord_music_maestro`
